...
function requireBrand() {
  ...
  const missingStrings = REQUIRED_STRING_FIELDS.filter((f) => typeof brand[f] !== "string" || brand[f].length === 0);
  if (missingStrings.length > 0) {
    throw new Error(`brands/${brandId}.json missing required string fields: ${missingStrings.join(", ")}`);
  }
  const invalidArrays = REQUIRED_ARRAY_FIELDS.filter((f) =>
    !Array.isArray(brand[f])
    || brand[f].length === 0
    || brand[f].some((item) => typeof item !== "string" || item.length === 0)
  );
  if (invalidArrays.length > 0) {
    throw new Error(`brands/${brandId}.json missing required non-empty string arrays: ${invalidArrays.join(", ")}`);
  }
  if (brand.id !== brandId) {
