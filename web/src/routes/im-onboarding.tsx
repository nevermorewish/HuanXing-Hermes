...
type ImSection = "overview" | ImPlatform | "dingtalk";
...
export function sectionFromPath(pathname: string): ImSection | null {
  if (pathname === "/im" || pathname === "/im/") return "overview";
...
function PlatformOverviewCard({ platform }: { platform: ImPlatform }) {
  ...
  return (
    <Link className={s.platformCard} to={`/im/${platform}`}>
...
function ConnectorRoute({ platform }: { platform: ImPlatform }) {
  const copy = PLATFORM_COPY[platform];
  const Icon = copy.icon;
  const stateQuery = useImOnboardingState(platform);
  const platformQuery = useMessagingPlatform(platform);
  ...
  return (
    <SectionShell title={`助理 · ${copy.name}`} sub="扫码接入">
      <div className={s.pageWrap}>
        <Link className={s.backLink} to="/im"><ArrowLeft size={15} />全部平台</Link>
        ...
        {checkedFailure && platform === "feishu" ? <FeishuFailureHelp result={checkedFailure} /> : null}
...
export function ImOnboardingRoute() {
  ...
  if (!section) return <Navigate to="/im" replace />;
